...
BNB_IN(5) vec4 lights[4];
BNB_IN(9) vec3 radiance[4];
#endif

BNB_IN(13) vec2 var_face_mask_uv;
BNB_DECLARE_SAMPLER_2D(10, 11, tex_face_mask);
...
void main()
{
 vec4 base_opacity = BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_diffuse),var_uv);
    ...
    vec3 base = g2l(js_color.xyz);
    float opacity = base_opacity.w;

    float metallic = js_metallic.x;
    float roughness = js_roughness.x;

#ifdef GLFX_TBN
vec3 normal = mix(BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_normal),var_uv).xyz, vec3(0.5,0.5,1.0), 1. - js_normal.x);
    vec3 N = normalize( mat3(var_t,var_b,var_n)*(normal*2.-1.) );
#else
    ...
    bnb_FragColor = vec4(l2g(color),opacity * js_opacity.x * face_mask);
}