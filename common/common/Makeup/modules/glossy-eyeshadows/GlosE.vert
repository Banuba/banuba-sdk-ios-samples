...
BNB_DECLARE_SAMPLER_2D(18, 19, bnb_UVMORPH_FISHEYE);
#endif
...
BNB_OUT(5) vec4 lights[4];
BNB_OUT(9) vec3 radiance[4];
BNB_OUT(13) vec2 var_face_mask_uv;
...
void main()
{
    ...
    lights[0] = vec4((bnb_MV*vec4(js_lights_eyeshadow_pos0.xyz,1.)).xyz - var_v, 0.);
    lights[1] = vec4((bnb_MV*vec4(js_lights_eyeshadow_pos1.xyz,1.)).xyz - var_v, 0.);
    lights[2] = vec4((bnb_MV*vec4(js_lights_eyeshadow_pos2.xyz,1.)).xyz - var_v, 0.);
    lights[3] = vec4((bnb_MV*vec4(js_lights_eyeshadow_pos3.xyz,1.)).xyz - var_v, 0.);

        radiance[0] = js_lights_eyeshadow_radiance0.xyz;
        radiance[1] = js_lights_eyeshadow_radiance1.xyz;
        radiance[2] = js_lights_eyeshadow_radiance2.xyz;
        radiance[3] = js_lights_eyeshadow_radiance3.xyz;
